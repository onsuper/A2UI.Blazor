...
public class DataBindingResolver
{
        ...
        string? dataContextPath = null)
    {
        ...
        foreach (var entry in contextEntries)
        {
            ...
            if (!entry.TryGetValue("value", out var valueObj) || valueObj is not Dictionary<string, object> value)
                continue;

            // Resolve the bound value
            var resolvedValue = ResolveBoundValue<object>(value, surfaceId, dataContextPath);
